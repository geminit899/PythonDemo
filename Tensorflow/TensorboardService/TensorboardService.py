...
class TensorboardHandler(web.RequestHandler):
    def get(self, name):
        if self.request.path.startswith('/' + name + '/htsc'):
            self.request.path = self.request.path[len('/' + name + '/htsc'):]
        elif self.request.path.startswith('/' + name):
            self.request.path = self.request.path[len('/' + name):]

        local_path = os.path.join(tmp_dir, 'log', name)
        ...
        tb_app = self.settings["manager"].new_instance(name, local_path, reload_interval=5)
